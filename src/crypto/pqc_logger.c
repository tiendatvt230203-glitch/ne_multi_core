...
void sig_pqc_write_log(int policy_id, const char *level, const char *status, const char *msg) {
    ...
    if (fp) {
        ...
        while(fgets(line_buf, sizeof(line_buf), fp) && saved_count < MAX_LOG_LINES) {
            ...
            if (sscanf(line_buf, "%d-%d-%dT%d:%d:%d%c%d:%d", &yr, &mo, &dy, &hr, &mi, &se, &tz_sign, &tz_hr, &tz_mi) == 9) {
                struct tm t;
                memset(&t, 0, sizeof(t));
                t.tm_year = yr - 1900;
                t.tm_mon = mo - 1;
                ...
                time_t long_time = mktime(&t);
                if (long_time != (time_t)-1) {
                    if (difftime(now, long_time) > 15.0 * 86400.0) {
                        keep = false;
                    }
                }
            }
    ...
    fp = fopen(LOG_FILE, "w");
    if (fp) {
        fprintf(fp, "%s [%s] [Policy: %d] Status: %s | MSG: %s\n", time_str, level, policy_id, status, msg);
