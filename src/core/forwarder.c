...
// #region agent log
static void agent_log_dataplane_config(void)
{
    const char *path = getenv("NE_DEBUG_LOG");
    FILE *df;
    long long ts;

    fprintf(stderr,
            "[DP-CONF] arch=unified-copy-tx-isolated RX(lan=%u wan=%u) TX(%u,%u,%u,%u) crypto=%u xsk=%s\n",
            (unsigned)NE_CPU_LOC, (unsigned)NE_CPU_WAN,
            (unsigned)NE_CPU_TX0, (unsigned)NE_CPU_TX1,
            (unsigned)NE_CPU_TX2, (unsigned)NE_CPU_TX3,
            (unsigned)NE_CRYPTO_WORKERS,
            (NE_XSK_BIND_FLAGS & XDP_COPY) ? "copy" : "other");
    fflush(stderr);

    if (!path)
        path = ".cursor/debug-250a01.log";
    df = fopen(path, "a");
    if (!df)
        return;
    ts = (long long)time(NULL) * 1000LL;
    fprintf(df,
            "{\"sessionId\":\"250a01\",\"hypothesisId\":\"H5\",\"location\":\"forwarder.c:forwarder_run\","
            "\"message\":\"dataplane config\",\"data\":{\"arch\":\"unified-copy-tx-isolated\",\"xsk\":\"copy\","
            "\"rx_slots\":%u,\"tx_slots\":%u,\"crypto\":%u},\"timestamp\":%lld}\n",
            (unsigned)NE_RX_SLOTS, (unsigned)NE_TX_SLOTS, (unsigned)NE_CRYPTO_WORKERS, ts);
    fclose(df);
}
// #endregion
