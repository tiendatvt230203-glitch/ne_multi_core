...
static const uint8_t local_rx_cpus[NE_RX_SLOTS] = {
    NE_CPU_LOC_RX0, NE_CPU_LOC_RX1,
};
static const uint8_t wan_rx_cpus[NE_RX_SLOTS] = {
    NE_CPU_WAN_RX0, NE_CPU_WAN_RX1,
};
static const uint8_t tx_cpus[NE_TX_SLOTS] = {
    NE_CPU_TX0, NE_CPU_TX1, NE_CPU_TX2, NE_CPU_TX3,
};
...
static void *local_rx_thread(void *arg)
{
    ...
    while (atomic_load_explicit(&running, memory_order_acquire)) {
        ...
        int rcvd = ne_recv_local_slot(&fwd->pair, batch, NE_BATCH_SIZE, rx_slot);
        if (rcvd <= 0) {
            sched_yield();
            continue;
        }
...
static void *wan_rx_thread(void *arg)
{
    ...
    while (atomic_load_explicit(&running, memory_order_acquire)) {
        ...
        int rcvd = ne_recv_wan_slot(&fwd->pair, batch, NE_BATCH_SIZE, rx_slot);
        if (rcvd <= 0) {
            sched_yield();
            continue;
        }
...
static void forwarder_join_started(struct forwarder *fwd, int local_rx_started,
                                   int tx_started, int crypto_started, int wan_rx_started)
{
...
void forwarder_run(struct forwarder *fwd)
{
    ...
    struct io_rx_slot_ctx local_rx_ctx[NE_RX_SLOTS];
    struct io_rx_slot_ctx wan_rx_ctx[NE_RX_SLOTS];
    ...
    for (int w = 0; w < (int)NE_RX_SLOTS; w++) {
        local_rx_ctx[w].fwd = fwd;
        local_rx_ctx[w].rx_slot = w;
        local_rx_ctx[w].cpu_id = local_rx_cpus[w];
    ...
    for (int w = 0; w < (int)NE_RX_SLOTS; w++) {
        ...
        wan_rx_ctx[w].cpu_id = wan_rx_cpus[w];
        if (pthread_create(&fwd->wan_rx_threads[w], NULL, wan_rx_thread, &wan_rx_ctx[w]) != 0) {
            forwarder_join_started(fwd, local_rx_started, tx_started, crypto_started, wan_rx_started);
            return;
        }
        wan_rx_started++;
    }
